...
Vue 3.5 + TypeScript(strict) + Vite 7 스타터입니다. ESLint(flat config),
Prettier, Vitest(+@vue/test-utils), Docker(nginx) 빌드를 포함합니다.

## 기술 스택

- Vue 3.5 (`<script setup lang="ts">`), TypeScript (strict)
- Vite 7 (`@` → `src` alias, `/api` 프록시 예시 포함)
- vue-tsc 타입 체크 (빌드 전 자동 실행)
...
| --- | --- |
| `npm run dev` | 개발 서버 |
| `npm run build` | 타입 체크(vue-tsc -b) 후 프로덕션 빌드 |
...
docker run --rm -p 8080:80 vue-vite-app
```
